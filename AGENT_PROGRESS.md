...
- Cancellation suppression, Stop-vs-unwind serialization, and idle native-cancellation suppression passed full Android CI and real Qwen E2E at `a18552b7`.

## Important work in progress
- Close the queued-generation lifecycle race with monotonic lifecycle epochs: generations requested before or during load/reset/unload remain stale after waiting for the generation mutex, while post-transition generations remain admissible. Implementation and focused epoch/overlap tests are on `agent-dev` pending exact-tip CI.
...
- `a18552b7` passed JVM tests, lint/debug APK build, emulator smoke, and the real Qwen LiteRT-LM E2E job.
- Earlier validated runtime hardening tips include `ec577e6b`, `9608b185`, `0b2d07a7`, `ddbce7f9`, `4e6e511a`, `9458e0d8`, `474269e0`, `9917f16f`, and `07c87115` through the same full Android CI pipeline.
- Current focused tests cover lifecycle permits captured before/during transitions, overlapping lifecycle transitions, transition preservation across generation-state reset, idle cancellation suppression, recoverable retry, duplicate suppression, and concurrent cancellation serialization.
...
- Lifecycle-epoch generation admission still needs exact-tip full CI and a real queued-generation-vs-reset/load/unload regression check.
