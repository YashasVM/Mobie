...
- Made final LiteRT load admission context-aware so c64k/c128k-style artifacts no longer pass native preflight under a 4K KV-cache assumption.
- Aligned runtime thermal admission with Android SEVERE pressure; exact-tip CI passed unit/lint/build, emulator smoke, and real Qwen E2E.

## In progress
- Configure LiteRT `EngineConfig.maxNumTokens` from the same inferred artifact context used by recommendation/load-memory admission. Large-context artifacts previously reserved RAM as c32k/c64k/etc. while the engine could still use its SDK/model default. Runtime wiring plus focused regression coverage are committed; exact-tip CI is pending.
...
- Latest validated tip `3c7f1613` passed JVM tests, lint/debug APK build, emulator smoke, and real Qwen LiteRT-LM E2E.
...
- Native context-window configuration is not yet CI-validated at the newest tip.
...
- Test c32k/c64k/c128k LiteRT artifacts and verify the configured native context, actual RAM use, and recommendation/load admission remain consistent on memory-constrained devices.
