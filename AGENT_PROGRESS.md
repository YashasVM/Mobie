...
## In progress
- Align catalog/model-detail presentation with the same device-selected artifact as the functional lifecycle. UI model snapshots now preserve all alternatives while pinning `bestArtifact` presentation to the selected generic LiteRT package; exact-tip CI is pending.
...
- Added JVM coverage that a device-preferred generic artifact controls presentation without deleting alternative variants, while a hardware-specific NPU artifact still cannot be forced into the generic path.
...
- Device-selected artifact presentation is implemented but remains pending exact-tip CI validation.
...
- Verify catalog cards and model details show the same package size/status/file that Mobie actually downloads after the presentation change clears CI.
