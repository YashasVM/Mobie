...
- Concurrent multi-artifact metadata publication fix is implemented on `agent-dev`: each worker now uses its own metadata temp file instead of sharing `.model.properties.part`; exact-tip Android CI is still running.
...
- Added a JVM regression asserting different concurrent metadata operations receive different sibling temp files; validation is pending on the current tip.
...
- The concurrent metadata publication fix is not yet CI-validated on the exact branch tip.
