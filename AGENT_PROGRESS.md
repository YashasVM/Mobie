...
- Guarding the production LiteRT stream against upstream Android failure modes where token callbacks stop without a terminal completion/error callback. The new runtime wrapper uses a long prefill allowance and a shorter active-stream idle watchdog, cancels the delegate, and converts missing terminal events into explicit failures rather than allowing Mobie to wait forever.
...
- New inference-stall regression coverage has been added for stalled prefill, mid-stream stalls, healthy pass-through, and streams that end without a terminal event; exact-tip CI is pending.
...
- Upstream LiteRT-LM Android streaming has open reports of missing terminal callbacks that can poison a conversation/engine; Mobie's watchdog mitigation is currently CI-gated and cannot yet be claimed to recover every native deadlock.
