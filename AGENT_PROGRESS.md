...
- Model download and checksum loops now cooperatively observe coroutine cancellation so Stop/delete requests do not leave foreground network or CPU work running until the next suspend point; exact-tip Android CI is pending.
...
- `18d4561f`: full Android CI passed manager-side metadata repair/verification collision protection: JVM tests/lint/debug APK, emulator instrumentation, real LiteRT-LM text E2E, and real LiteRT-LM vision E2E.
...
- Cooperative cancellation inside blocking download/checksum loops is not yet CI-validated on the exact branch tip.
